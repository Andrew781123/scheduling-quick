    ...
    while(i < inputTimeSlots.length && j < matchedTimeSlots.length) {
      ...
      if(checkInBetween(inputStart, matchedStart, inputEnd, matchedEnd)) {
        ...
        if(temp.length == 0) {
          ...
          if(largerRange) {
            temp.push(matchedTimeSlots[j])
          } else {
            temp.push(inputTimeSlots[i])
          }
        }
...
const splitTimeSlots = (timeSlots) => {
  ...
  for(let i = 0; i < splittedTimeSlots.length; i++) {
    ...
    for(let j = 0; j < timeSlots.length; j++) {
      ...
      if(checkInBetween(splittedStart, originalStart, splittedEnd, originalEnd)) {
        const people = timeSlots[j][2];
        if(splittedTimeSlots[i][2]) splittedTimeSlots[i][2] = [...splittedTimeSlots[i][2], ...people];
